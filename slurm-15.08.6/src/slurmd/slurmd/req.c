...
int
simulator_add_future_event(batch_job_launch_msg_t *req)
{
...
simulator_rpc_batch_job(slurm_msg_t *msg)
{
	...
	if (!_slurm_authorized_user(req_uid)) {
		error("REQUEST_BATCH_JOB_LAUNCH request from uid %u", req_uid);
		return;
	}
...
simulator_rpc_terminate_job(slurm_msg_t *rec_msg)
{
	...
	hostlist_t hl;
	char *node_name;
	int             rc     = SLURM_SUCCESS, found = 0;
	kill_job_msg_t *req_kill    = rec_msg->data;
	volatile simulator_event_t *temp, *event_sim;
	...
	info("simulator_rpc_terminate_job, jobid = %d ready for sending message",
					req_kill->job_id);
	...
	if ((head_sim_completed_jobs) &&
	    (head_sim_completed_jobs->job_id == req_kill->job_id)) {
		found = 1;
	} else {
	...
	if ( found ) {
		...
		node_name = strdup("<NULL>");
	}

	info("SIM: Sending epilog complete message for job %d node %s",
					req_kill->job_id, node_name);
	slurm_msg_t_init(&msg);
