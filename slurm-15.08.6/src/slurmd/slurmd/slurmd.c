...
#ifdef SLURM_SIMULATOR
/* The following three is for some global synchronization logic */
#include <semaphore.h>
...
extern void      sim_perform_slurmd_register(char * sem_name,
					sem_t ** mutex_sync);
extern void      sim_perform_global_sync(char * sem_name, sem_t ** mutex_sync);
#endif
...
#ifdef SLURM_SIMULATOR
/* Notify the sim_mgr that this slurmd is ready to continue */
static int
notify_sim_mgr()
{
	int sem_opened = 0;
...
_send_sim_helper_cycle_msg(uint32_t jobs_count)
{
	...
	if (i > 5) {
		sleep(10);
		error("SIM: Unable to send message helper cycle complete "
							"message: %m");
		return SLURM_ERROR;
	}
...
{
	time_t now, last;
	int jobs_ended, sem_opened;

	_increment_thd_count();

	sem_opened = sim_open_sem(syn_sem_name, &mutexserver, 0);
	...
	while (!_shutdown) {
		...
		while ((head_simulator_event) &&
			...
			info("SIM: Sending JOB_COMPLETE_BATCH_SCRIPT"
				" for job %d", event_jid);
			...
			info("SIM: JOB_COMPLETE_BATCH_SCRIPT for "
		...
		if(jobs_ended) {
			/*
			 * Let's give some time to EPILOG_MESSAGE process
			 * to terminate
			 * TODO: It should be done better with a counter
			 * of EPILOG messages processed */
			usleep(5000);
		...
		/* sem_open previously failed, try again */
		if (sem_opened)
			sem_opened = sim_open_sem(syn_sem_name, &mutexserver, 0);

		if (!sem_opened)
			sim_perform_global_sync(syn_sem_name, &mutexserver);

	}
	info("SIM: Simulator Helper finishing...");

	if (!sem_opened) sim_close_sem(&mutexserver);
	_decrement_thd_count();
	pthread_exit(0);
	return NULL;
}
